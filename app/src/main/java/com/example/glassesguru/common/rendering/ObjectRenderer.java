package com.example.glassesguru.common.rendering;
...
public class ObjectRenderer {
  ...
  private float[] customColor = DEFAULT_COLOR;
  private boolean isCustomColorSet = false;
  ...
  public void setUseDepthForOcclusion(Context context, boolean useDepthForOcclusion) throws IOException {
  ...
  private void compileAndLoadShaderProgram(Context context) throws IOException {
    ...
    // New uniform for custom color
    customColorUniform = GLES20.glGetUniformLocation(program, "u_CustomColor");
  ...
  public void draw(float[] cameraView, float[] cameraPerspective, float[] colorCorrectionRgba) {
    if (!isCustomColorSet) {
      ...
      draw(cameraView, cameraPerspective, colorCorrectionRgba, customColor);
    }
  }
  ...
  public void draw(float[] cameraView, float[] cameraPerspective, float[] colorCorrectionRgba, float[] objColor) {
    ...
    GLES20.glUniform4fv(customColorUniform, 1, customColor, 0);
  ...
  private void normalizeVec3(float[] v) {
    float reciprocalLength = 1.0f / lengthVec3(v);
    v[0] *= reciprocalLength;
    v[1] *= reciprocalLength;
    v[2] *= reciprocalLength;
  }

  private float lengthVec3(float[] v) {
    return (float) Math.sqrt(v[0] * v[0] + v[1] * v[1] + v[2] * v[2]);
  }

  public void setDepthTexture(int depthTextureId, float[] uvTransform, float depthAspectRatio) {
    this.depthTextureId = depthTextureId;
    this.uvTransform = uvTransform;
    this.depthAspectRatio = depthAspectRatio;
  }
